...
public class AttributeUpdateDTO {
    ...
    @NotBlank
    @Size(min = 2, max = 255, message = "Attribute name must be between {min} and {max} character length")
    private String name;

    @Schema(description = "Attribute data type", example = "pdf")
    @NotBlank
    @Size(min = 2, max = 255, message = "Attribute data type must be between {min} and {max} character length")
