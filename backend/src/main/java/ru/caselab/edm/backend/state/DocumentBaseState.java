...
public class DocumentBaseState implements DocumentState {
    ...
    public void signAuthor(DocumentVersion version) {
        ApprovementProcess process = version.getApprovementProcesses().getLast();
        if(process!=null){
            switch (process.getStatus()){
                case PUBLISHED_FOR_VOTING -> process.setStatus(ApprovementProcessStatus.PUBLISHED_FOR_VOTING);
            ...
            switch (status) {
                ...
                default ->
                        throw new InvalidDocumentStateException("Action - sign by Author is not allowed in the current state.");
            }
        }
    }
    ...
    public void publishForVoting(DocumentVersion version) {
        if(isVotingDocument(version)){
            ...
            throw new InvalidDocumentStateException("Action - publish for voting is not allowed in the current state.");
        }
    }
    ...
    private boolean isVotingDocument(DocumentVersion version) {
        return version.getApprovementProcesses()!=null;
    }
